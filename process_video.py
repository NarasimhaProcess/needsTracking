...
def fix_cookies_format(file_path):
    """Fix cookie file formatting if needed."""
...
Do not include timestamps, metadata, speaker labels,
explanations, descriptions, or transcription notes.

Keep the natural meaning and flow of the original speech.

If the target language is Indian English, use natural
Indian English phrasing and idioms where appropriate.
"""
    ...
    video_duration = get_duration(downloaded_video)
    audio_duration = get_duration(translated_audio)
    ...
    if 0.5 <= speed_ratio <= 2.0:
        ...
        print(
            "WARNING: Speed ratio outside safe range."
        )
    ...
    # --------------------------------------------------
    # Merge video and translated audio
    # --------------------------------------------------
